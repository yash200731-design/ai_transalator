...
async function startServer() {
  ...
  app.get("/api/health", (req, res) => {
    res.json({ status: "ok", geminiActive: !!process.env.GEMINI_API_KEY });
  });

  // API route for performing actual high-fidelity Gemini translations
  app.post("/api/translate", async (req, res) => {
    ...
    try {
      ...
      // Formulate prompt for high-quality Gemini 2.5 Flash translation
...
"${text}"`;
      ...
      // Call Gemini 2.5 Flash with structured response schema
      ...
      if (!responseText) {
        throw new Error("Empty response received from the Gemini Translation API");
      }
      ...
      return res.status(500).json({
        error: error.message || "An error occurred while calling the Gemini API. Please make sure your GEMINI_API_KEY is configured."
      });
    }
  });
